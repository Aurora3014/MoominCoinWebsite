...
const Header: FC = memo(() => {
  ...
  const navSections = useMemo(
    () => [SectionId.About, SectionId.Testimonials, SectionId.Resume, SectionId.Portfolio],
    [],
  );
...
const DesktopNav: FC<{navSections: SectionId[]; currentSection: SectionId | null}> = memo(
  ({navSections, currentSection}) => {
    const baseClass =
      '-m-1.5 p-1.5 rounded-md font-bold first-letter:uppercase hover:transition-colors hover:duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 sm:hover:text-[#C72727] text-neutral-100';
    const activeClass = classNames(baseClass, 'text-neutral-900 font-poppins text-md  header-text p-4');
    const inactiveClass = classNames(baseClass, 'text-neutral-900 font-poppins text-md  header-text p-4');
    return (
      <header className="fixed top-0 z-50 hidden w-full pt-4 pb-4 backdrop-blur sm:block bg-[#fff]/25 flex flex-row justify-between" id={headerID}>
...
const MobileNav: FC<{navSections: SectionId[]; currentSection: SectionId | null}> = memo(
  ({navSections, currentSection}) => {
    ...
    const activeClass = classNames(baseClass, 'bg-neutral-900 font-bold');
