    ...
    <Section noPadding sectionId={SectionId.Testimonials}>
      <div
        ...
        <div className="z-10 w-full max-w-screen-md px-4 lg:px-0">
          <div className="flex flex-col items-center gap-y-6 rounded-xl bg-gray-900/60 p-6 shadow-lg">
            <div
...
const Testimonial: FC<{testimonial: Testimonial; isActive: boolean}> = memo(
  ({testimonial: {text, name, image}, isActive}) => (
    <div
      ...
      <div className="relative md:h-48 md:w-48 shrink-0 sm:h-48 sm:w-48">
        <Image alt="moomin-family-image"  className="md:h-full md:w-full rounded-full xs:h-14 xs:w-14" src={image!} />
      </div>

      <div className="flex flex-col h-full justify-center gap-y-4">
        <p className="prose prose-sm font-medium italic text-white md:text-xl text-xl sm:prose-base">{text}</p>
        <p className="text-xs italic text-white sm:text-sm md:text-base lg:text-xl">-- {name}</p>
      </div>
    </div>
  ),
);
